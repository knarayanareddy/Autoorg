...
import { RiskEngine, type RiskTier } from './risk-engine.js';
...
export class PolicyEngine {
  ...
  seedDefaults(teamId?: string) {
    const defaults = [
      ...
      // EXECUTE
      ['Engineer', 'EXECUTE', '*', 'medium', 1, 0, 1, 1, 1],
  ...
  evaluate(intent: ActionIntent): PolicyDecision {
    ...
    const rows = db.prepare(`
      ...
      ORDER BY CASE WHEN role = '*' THEN 1 ELSE 0 END, updated_at DESC
    `).all(
      this.runId,
      intent.role,
      intent.actionClass,
    ...
    if (!rule) {
      return {
        ...
        reasons: [`No explicit policy for ${intent.role}/${intent.actionClass}/${intent.targetRef}`],
      };
    }
